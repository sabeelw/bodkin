use crate::fmt::{bps, eth, hhmmss, pad, short, usd};
use crate::links::{osc, Links};
...
pub fn launch_card(intel: &LaunchIntel, score: &Score, ctx: &ViewCtx, links: &Links) -> String {
    let ev = &intel.ev;
    let name = intel.meta.as_ref().map(|m| m.name.as_str()).unwrap_or("(unreadable)");
    let sym = intel.meta.as_ref().map(|m| format!("${}", m.symbol)).unwrap_or_default();
    let ts = intel.tx.as_ref().map(|t| hhmmss(Some(t.timestamp))).unwrap_or_else(|| hhmmss(None));
    ...
    let dev = intel.tx.as_ref().map(|tx| {
        format!(
            "{:.2}% ({:.4} {})",
    ...
    let tax = intel.record.as_ref().map(|r| bps(r.creator_tax_bps as u64)).unwrap_or_else(|| "?".into());
    let fee_to = match (&intel.record, &intel.tx) {
        (Some(rec), Some(tx)) if rec.creator_fee_recipient == tx.from => "deployer".into(),
        (Some(rec), Some(_)) => format!("{} {}", short(&format!("{:#x}", rec.creator_fee_recipient), 4), neon("third party")),
        _ => "?".into(),
    };
    ...
    lines.push(format!(
        ...
        white(dev),
        white(tax),
        if ex > 0 { loss(ex.to_string()) } else { white("0") }
    ));

    let soc = has_socials(intel.meta.as_ref());
    let mut soc_bits = Vec::new();
    if soc.twitter { soc_bits.push("x"); }
    if soc.website { soc_bits.push("web"); }
    if soc.telegram { soc_bits.push("tg"); }
    let soc_txt = if soc_bits.is_empty() { loss("none") } else { soc_bits.join(" ") };
    let desc = intel.meta.as_ref().map(|m| m.description.replace('\n', " ")).unwrap_or_default();
    let desc = desc.trim();
    let desc = if desc.len() > 96 { format!("{}…", &desc[..96]) } else { desc.to_string() };
    ...
    if let Some(curve_s) = &intel.curve {
        if intel.record.as_ref().is_some_and(|r| r.phase != 0) {
            let phase = ["curve", "swept", "v4 pool", "rescued"][intel.record.as_ref().map(|r| r.phase.min(3) as usize).unwrap_or(0)];
            lines.push(format!("   {} → {phase}  {}", neon("graduated"), muted("curve reserves are empty after graduation; price now lives in the pool")));
        } else {
            ...
            let open = if !curve_s.opening_tax_bps.is_zero() {
                loss(format!("opening tax {}", bps(curve_s.opening_tax_bps.try_into().unwrap_or(0))))
            } else {
                muted("opening tax 0")
            };
    ...
    if let Some(a) = &ctx.activity {
        lines.push(format!(
            ...
            a.buys, a.sells, a.unique_buyers, a.taxed_buys, eth(a.quote_in.saturating_sub(a.quote_out))
        ));
    }
    lines.push(format!("   {}", muted(score.reasons.join(" · "))));
    let open = [
    ...
    .collect::<Vec<_>>()
    .join(" · ");
    lines.push(format!("   {} {open}   {}", muted("open:"), muted("ctrl+click in Windows Terminal, iTerm, kitty, VS Code")));
    let _ = links;
    lines.join("\n")
}

pub fn launch_update(intel: &LaunchIntel, score: &Score, activity: &CurveActivity, age_sec: u64, ctx: &ViewCtx) -> String {
    let sym = intel.meta.as_ref().map(|m| format!("${}", m.symbol)).unwrap_or_else(|| short(&format!("{:#x}", intel.ev.token), 4));
    ...
    format!(
        ...
        muted(hhmmss(None)),
        pad(&sym, 10),
        pad(&format!("{age_sec}s"), 4),
        verdict_tag(score.verdict),
        format!("{:>3}", score.total),
        p * 100.0,
        fdv,
        muted(usd(fdv_usd)),
        activity.buys,
